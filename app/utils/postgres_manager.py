...
EOF"''')
    ...
    def _create_default_pg_hba(self, target_path):
        """Create a default pg_hba.conf file if it doesn't exist
        
        Args:
            target_path (str): Path where pg_hba.conf should be created
        ...
        # Define default pg_hba.conf content
        default_content = """# Generated by NexPostgres
...
"""
        ...
        with open('/tmp/default_pg_hba.conf', 'w') as f:
            f.write(default_content)
            
        # Upload the file to the server
        upload_result = self.ssh.upload_file('/tmp/default_pg_hba.conf', '/tmp/default_pg_hba.conf')
        ...
        create_dir = self.ssh.execute_command(f"sudo mkdir -p {dir_path}")
        if create_dir['exit_code'] != 0:
            self.logger.error(f"Failed to create directory {dir_path}: {create_dir['stderr']}")
            return False
            
        move_result = self.ssh.execute_command(f"sudo cp /tmp/default_pg_hba.conf {target_path}")
    ...
    def check_and_fix_external_connections(self):
        """
        Check and fix PostgreSQL configuration to ensure it listens on external IPs and allows external connections.
        Updates postgresql.conf to set listen_addresses = '*' and modifies pg_hba.conf to allow external connections.
        
        Returns:
            tuple: (success, message, changes_made)
                - success (bool): Whether the operation was successful
                - message (str): A message describing the result
                - changes_made (dict): Dictionary of changes made to the configuration
        """
        changes_made = {
        ...
        self.logger.info("Starting external connections configuration check...")
        
        # Find PostgreSQL configuration file
        pg_conf = self._find_postgresql_conf()
        if not pg_conf:
            self.logger.error("Could not locate PostgreSQL configuration file")
            return False, "Could not locate PostgreSQL configuration file", changes_made
        else:
            self.logger.info(f"Found postgresql.conf at: {pg_conf}")
        
        # Find pg_hba.conf file using multiple search methods
        pg_hba_path = None
        search_attempts = []
        
        # Method 1: Check in same directory as postgresql.conf
        conf_dir = os.path.dirname(pg_conf)
        possible_pg_hba = os.path.join(conf_dir, "pg_hba.conf")
        self.logger.info(f"Looking for pg_hba.conf at: {possible_pg_hba}")
        check_hba = self.ssh.execute_command(f"sudo test -f {possible_pg_hba} && echo 'exists'")
        if 'exists' in check_hba['stdout']:
            pg_hba_path = possible_pg_hba
            self.logger.info(f"Found pg_hba.conf at: {pg_hba_path} (same dir as postgresql.conf)")
        else:
            search_attempts.append(f"Not found at: {possible_pg_hba}")
        
        # Method 2: Check in data directory
        if not pg_hba_path:
            self.logger.info("Trying to find pg_hba.conf in data directory...")
            data_dir = self.get_data_directory()
            if data_dir:
                possible_pg_hba = os.path.join(data_dir, "pg_hba.conf")
                self.logger.info(f"Looking for pg_hba.conf at: {possible_pg_hba}")
                check_hba = self.ssh.execute_command(f"sudo test -f {possible_pg_hba} && echo 'exists'")
                if 'exists' in check_hba['stdout']:
                    pg_hba_path = possible_pg_hba
                    self.logger.info(f"Found pg_hba.conf at: {pg_hba_path} (in data directory)")
                else:
                    search_attempts.append(f"Not found at: {possible_pg_hba}")
            else:
                search_attempts.append("Could not determine data directory")
        
        # Method 3: Ask PostgreSQL directly (most reliable)
        if not pg_hba_path:
            self.logger.info("Trying to find pg_hba.conf by querying PostgreSQL...")
            result = self.ssh.execute_command("sudo -u postgres psql -t -c 'SHOW hba_file;'")
            if result['exit_code'] == 0 and result['stdout'].strip():
                possible_pg_hba = result['stdout'].strip()
                self.logger.info(f"PostgreSQL reports hba_file as: {possible_pg_hba}")
                check_hba = self.ssh.execute_command(f"sudo test -f {possible_pg_hba} && echo 'exists'")
                if 'exists' in check_hba['stdout']:
                    pg_hba_path = possible_pg_hba
                    self.logger.info(f"Found pg_hba.conf at: {pg_hba_path} (from SHOW hba_file)")
                else:
                    search_attempts.append(f"File reported by PostgreSQL not found: {possible_pg_hba}")
                    # Try to create the file if PostgreSQL is looking for it
                    self.logger.info(f"PostgreSQL expects pg_hba.conf at {possible_pg_hba} but file doesn't exist. Attempting to create it.")
                    if self._create_default_pg_hba(possible_pg_hba):
                        pg_hba_path = possible_pg_hba
                        changes_made['created_files'] = True
                        self.logger.info(f"Created new pg_hba.conf at: {pg_hba_path}")
            else:
                if result['stderr']:
                    search_attempts.append(f"Error querying PostgreSQL: {result['stderr']}")
                else:
                    search_attempts.append("No output from PostgreSQL SHOW hba_file query")
        
        # Method 4: Common locations based on detected version
        if not pg_hba_path:
            self.logger.info("Trying to find pg_hba.conf in version-specific paths...")
            pg_version = self.get_postgres_version()
            if pg_version:
                self.logger.info(f"Detected PostgreSQL version: {pg_version}")
                # For PostgreSQL 17 specifically
                if pg_version.startswith('17'):
                    search_paths = [
                        f"/etc/postgresql/17/main/pg_hba.conf",
                        f"/var/lib/postgresql/17/main/pg_hba.conf",
                        f"/etc/postgresql/{pg_version}/main/pg_hba.conf",
                        f"/var/lib/postgresql/{pg_version}/main/pg_hba.conf"
                    ]
                else:
                    search_paths = [
                        f"/etc/postgresql/{pg_version}/main/pg_hba.conf",
                        f"/var/lib/postgresql/{pg_version}/data/pg_hba.conf",
                        f"/var/lib/pgsql/{pg_version}/data/pg_hba.conf"
                    ]
                
                for path in search_paths:
                    self.logger.info(f"Looking for pg_hba.conf at: {path}")
                    check_hba = self.ssh.execute_command(f"sudo test -f {path} && echo 'exists'")
                    if 'exists' in check_hba['stdout']:
                        pg_hba_path = path
                        self.logger.info(f"Found pg_hba.conf at: {pg_hba_path} (from version-specific paths)")
                        break
                    else:
                        search_attempts.append(f"Not found at: {path}")
                        
                # If still not found and we know the PostgreSQL version,
                # try to create the file in the most common location
                if not pg_hba_path:
                    if pg_version.startswith('17'):
                        target_path = f"/etc/postgresql/17/main/pg_hba.conf"
                        self.logger.info(f"No pg_hba.conf found. Creating one at default location: {target_path}")
                        if self._create_default_pg_hba(target_path):
                            pg_hba_path = target_path
                            changes_made['created_files'] = True
                            self.logger.info(f"Created new pg_hba.conf at: {pg_hba_path}")
                    else:
                        target_path = f"/etc/postgresql/{pg_version}/main/pg_hba.conf"
                        self.logger.info(f"No pg_hba.conf found. Creating one at default location: {target_path}")
                        if self._create_default_pg_hba(target_path):
                            pg_hba_path = target_path
                            changes_made['created_files'] = True
                ...
                search_attempts.append("Could not determine PostgreSQL version")
        
        # Method 5: Find with grep on common paths
        if not pg_hba_path:
            self.logger.info("Trying to find pg_hba.conf using find command...")
            result = self.ssh.execute_command("sudo find /etc/postgresql /var/lib/postgresql /var/lib/pgsql -name pg_hba.conf 2>/dev/null | head -1")
            if result['exit_code'] == 0 and result['stdout'].strip():
                pg_hba_path = result['stdout'].strip()
                self.logger.info(f"Found pg_hba.conf at: {pg_hba_path} (using find)")
            else:
                search_attempts.append("Not found using find command")
                
                # Output the stderr from the find command for debugging
                if result['stderr']:
                    self.logger.error(f"Find command error: {result['stderr']}")
        
        # Last resort - create pg_hba.conf if we have a data directory
        if not pg_hba_path:
            data_dir = self.get_data_directory()
            if data_dir:
                target_path = os.path.join(data_dir, "pg_hba.conf")
                self.logger.info(f"Last resort: Creating pg_hba.conf in data directory: {target_path}")
                if self._create_default_pg_hba(target_path):
                    pg_hba_path = target_path
                    changes_made['created_files'] = True
                    self.logger.info(f"Created new pg_hba.conf at: {pg_hba_path}")
        ...
        if not pg_hba_path:
            ...
            self.logger.error("Could not locate or create pg_hba.conf file. Gathering debug information...")
            
            # Check if PostgreSQL is running
            pg_status = self.ssh.execute_command("sudo systemctl status postgresql || ps aux | grep postgres | grep -v grep")
            if pg_status['stdout']:
                self.logger.info(f"PostgreSQL status: {pg_status['stdout']}")
                
            # List PostgreSQL related files
            list_pg_files = self.ssh.execute_command("sudo find /etc /var/lib -name '*post*' | grep -v 'postfix\\|postupgrade' 2>/dev/null | sort")
            if list_pg_files['stdout']:
                self.logger.info(f"PostgreSQL related files: \n{list_pg_files['stdout']}")
                
            # Format error message with search attempts
            error_details = "\n".join(search_attempts)
            return False, f"Could not locate or create pg_hba.conf file. Search attempts:\n{error_details}", changes_made
        
        self.logger.info(f"Using pg_hba.conf at: {pg_hba_path}")
            
        # Backup configuration files
        if not changes_made['created_files']:  # Only backup if we didn't create the file
            self.ssh.execute_command(f"sudo cp {pg_conf} {pg_conf}.bak.$(date +%Y%m%d%H%M%S)")
            self.ssh.execute_command(f"sudo cp {pg_hba_path} {pg_hba_path}.bak.$(date +%Y%m%d%H%M%S)")
        
        # Check and update listen_addresses in postgresql.conf
        check_listen = self.ssh.execute_command(f"sudo grep -E '^[ \\t]*listen_addresses[ \\t]*=' {pg_conf}")
        
        if check_listen['exit_code'] == 0:
            # Check if already set to '*'
            if "'*'" in check_listen['stdout'] or '"*"' in check_listen['stdout']:
                self.logger.info("listen_addresses already set to '*'")
            else:
                # Update existing setting
                self.ssh.execute_command(f"sudo sed -i 's|^[ \\t]*listen_addresses[ \\t]*=.*|listen_addresses = \\'*\\'|' {pg_conf}")
                changes_made['listen_addresses'] = True
                self.logger.info("Updated listen_addresses to '*'")
        else:
            # Add new setting
            self.ssh.execute_command(f"echo \"listen_addresses = '*'\" | sudo tee -a {pg_conf}")
            changes_made['listen_addresses'] = True
            self.logger.info("Added listen_addresses = '*' to postgresql.conf")
        
        # Check and update pg_hba.conf to allow external connections
        # First check if it already has external IP configuration
        check_external = self.ssh.execute_command(f"sudo grep -E '^host[ \\t]+all[ \\t]+all[ \\t]+0\\.0\\.0\\.0/0[ \\t]+' {pg_hba_path}")
        
        if check_external['exit_code'] != 0:
            # No external IP configuration found, add it
            self.ssh.execute_command(f"echo 'host    all    all    0.0.0.0/0    md5' | sudo tee -a {pg_hba_path}")
            changes_made['pg_hba'] = True
            self.logger.info("Added IPv4 external access rule to pg_hba.conf")
        else:
            self.logger.info("External IPv4 access already configured in pg_hba.conf")
            
        # Check if IPv6 access is configured
        check_ipv6 = self.ssh.execute_command(f"sudo grep -E '^host[ \\t]+all[ \\t]+all[ \\t]+::/0[ \\t]+' {pg_hba_path}")
        
        if check_ipv6['exit_code'] != 0:
            # No IPv6 configuration found, add it
            self.ssh.execute_command(f"echo 'host    all    all    ::/0    md5' | sudo tee -a {pg_hba_path}")
            changes_made['pg_hba'] = True
            self.logger.info("Added IPv6 external access rule to pg_hba.conf")
        else:
            self.logger.info("External IPv6 access already configured in pg_hba.conf")
            
        # Restart PostgreSQL if changes were made
        if changes_made['listen_addresses'] or changes_made['pg_hba'] or changes_made['created_files']:
            self.logger.info("Configuration changes made, restarting PostgreSQL...")
            if self._restart_postgres():
                if changes_made['created_files']:
                    return True, "Created missing PostgreSQL configuration files and configured for external connections", changes_made
                else:
                    return True, "PostgreSQL configuration updated to allow external connections", changes_made
            else:
